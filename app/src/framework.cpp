...
bool Framework::init(const char *title, int xpos, int ypos, int width,
                     int height) {
  // Setup SDL
  ...
  if (SDL_GL_SetSwapInterval(-1) != 0)
    SDL_GL_SetSwapInterval(
        0); // Enables Adaptive v-sync if possible, otherwise v-sync
  ...
  io.ConfigDockingTransparentPayload =
      true; // Enable Transparent Window on Docking
...
void Framework::loadImageRGB(TextureData &texture, std::string path) {
  ...
  constexpr int formatRGB24 =
      386930691; // a magic number you might say. constexpr is const but better.
...
void Framework::loadImageRGBA(TextureData &texture, std::string path) {
  ...
  texture.height = image->h;
  texture.data =
      std::vector<GLubyte>(texture.width * texture.height * fmt->BytesPerPixel);
  int temp = 0;
...
void Framework::mouseDraw() {
  ...
  game->mouseDraw(state.mouseX, state.mouseY, state.drawSize, state.drawChance,
                  state.drawMaterial, state.drawShape);
}
