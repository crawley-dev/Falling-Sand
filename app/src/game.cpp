...
void Game::simulate(AppState& state) {
    ...
    if (mouseChunk->cells.size() > CHUNK_SIZE * CHUNK_SIZE || mouseChunk->cells.size() < 0) {
        mouseChunk = getChunk(0, 0);
    }
    if (outOfTextureBounds(state.mouse.x, state.mouse.y)) {
        state.mouse = Vec2<u16>(0, 0);
    }
    ...
    if (state.scanMode == Scan::BOTTOM_UP_L || state.scanMode == Scan::BOTTOM_UP_R) {
        state.scanMode = state.frame % 2;
    }
    state.frame++;
}
...
bool Game::updateChunk(Chunk* chunk, bool updateLeft) {
...
bool Game::updateCell(Cell& c, s32 x, s32 y) {
...
bool Game::updateSand(Cell& c, s32 x, s32 y) {
    s8 yDispersion = 0;
    s8 xDispersion = 0;
...
bool Game::updateWater(Cell& c, s32 x, s32 y) {
    s8 yDispersion = 0;
    s8 xDispersion = 0;
    ...
    while (movesLeft > 0) {
        ...
        /// TODO: optimise out some branches, could probably remove if (getRand)
...
bool Game::updateNaturalGas(Cell& c, s32 x, s32 y) {
    s8 yDispersion = 0;
    s8 xDispersion = 0;
...
// shoud be very slow ..
// optimise for changing cells in the same chunk
...
// avoid a branch for more maths when grabbing chunks?
...
void Game::updateTexture(std::vector<u8>& textureData) {
    //for (auto& [cell, coord] : cellUpdates) {
...
Cell& Game::getCell(Vec2<s32> coord) {
    Vec2<s16> chunkVec2 = worldToChunk(coord.x, coord.y);
    Chunk*    chunk     = getChunk(chunkVec2);
    return chunk->cells[cellIdx(worldToCell(chunkVec2.x, chunkVec2.y, coord.x, coord.y))];
}

Material& Game::getMaterial(Cell c) {
